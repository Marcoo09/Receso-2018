...
def parse_arguments():
  ...
  group.add_argument('--username', type=str,default = "maarco09", help='The twitter screen name from the account we want to retrieve all the pictures')
  group.add_argument('--hashtag', type=str, help='The twitter tag we want to retrieve all the pictures. ')
  parser.add_argument('--num', type=int, default=200, help='Maximum number of tweets to be returned.')
...
def tweet_media_urls(tweet_status):
  for s in tweet_status:

  	media = s.entities.get('media', [])
  	if(len(media) > 0):
...
def download_images_by_user(api, username, retweets, replies, num_tweets, output_folder):
  ...
  download_images("Test", num_tweets, output_folder)
...
def download_images_by_tag(api, tag, retweets, replies, num_tweets, output_folder):
  ...
  download_images("Test1", num_tweets, output_folder)
